<%@ page pageEncoding="UTF-8" %>
<div class="right" id="c_right">
    <div class="s_about">
        <h2>关于博主</h2>
        <img src="http://localhost:8080/upload/${sessionScope.SESSION_USER.avatar}" width="230" height="230" alt="博主"/>
        <p>博主：${sessionScope.SESSION_USER.userName}</p>
        <p>职业：${sessionScope.SESSION_USER.job}</p>
        <div class="clear"></div>
    </div>
    <div class="lanmubox">
        <div class="hd">
            <ul><li>精心推荐</li></ul>
        </div>
        <div class="bd">
            <ul>
                <li><a href="blog_1.jsp" >PayPal支付对接</a></li>
                <li><a href="blog_2.jsp" >jQuery选择器</a></li>
                <li><a href="blog_3.jsp" >如何解决80端口被pid=4的进程占用</a></li>
                <li><a href="blog_4.jsp" >Java多态详解</a></li>
                <li><a href="blog_5.jsp" >java基础三大特性</a></li>
            </ul>
        </div>
    </div>
    <div class="link">
        <h2>友情链接</h2>
        <p><a href="http://blog.csdn.net/?ref=toolbar_logo">CSDN博客</a></p>
        <p><a href="http://www.iteye.com/">ITeye</a></p>
    </div>
</div>
<script type="text/javascript">jQuery(".lanmubox").slide({easing:"easeOutBounce",delayTime:400});</script>
<script  type="text/javascript" src="js/nav.js"></script>
